...
const SearchBar = ({ user }) => {
  ...
  return (
    <>
      <SPCContainer
        className="private_chat"
        isInputFocus={isInputFocus}
        handlers={{ SELECT_SPC: (e) => console.log(e) }}
      >
        <SPCInput
          type="text"
          ...
          <SPCSContainer className="hello">
            {dmSearch.suggestions.map((suggestion) =>
              ...
              suggestion.username !== user.username ? (
                <SPCS>
                  <SPCSImage src={suggestion.image_url} alt="" />
                  <SPCSFriendName to={`/dm/${suggestion.username}`}>
                    {suggestion.username}
                  </SPCSFriendName>
                </SPCS>
              ) : null
